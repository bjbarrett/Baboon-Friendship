...
par(mar=c(1,0,0,0))
plot( DSI[d$nodad==1] ~ s_rank[d$nodad==1] , data=d , col=alpha("red",0.5),pch=16 ,ylim=c(0,11),ylab='',xlab='',xlim=c(min(d$s_rank),max(d$s_rank)), xaxt='n')
lines( rank.seq , pred.median2 )
text(0,11,"b. other males", cex=1.2)
...
par(mar=c(1,0,0,0))
plot( DSI[d$nextdad==1] ~ s_rank[nextdad==1] , data=d , col=alpha("orange",0.5),pch=16 ,ylim=c(0,11),ylab='',xlab='',xlim=c(min(d$s_rank),max(d$s_rank)),xaxt='n')
lines( rank.seq , pred.median3 )
text(0,11,"c. male sires next infant", cex=1.2)
